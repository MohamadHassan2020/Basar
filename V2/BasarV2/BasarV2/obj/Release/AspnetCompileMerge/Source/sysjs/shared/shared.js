class Shared {
    ...
    static getUrl() {
        ...
        return 'https://help.albasarint.com';
    }
    ...
    getStatusTextDataTable(status = false, trueText = 'نشط', falseText = 'غير نشط', nullText = '', trueClass = 'badge-light-success', falseClass = 'badge-light-danger',  nullClass = 'badge-light-primary') {
        if (status) {
            return `<span class="badge ${trueClass}">${trueText}</span>`;
        }
        if (status === false) {
            return `<span class="badge ${falseClass}">${falseText}</span>`;
        }
        else {
            return `<span class="badge ${nullClass}">${nullText}</span>`;
        }
    }
    ...
    async getRowByClassClick(succ, classClick ='.delete-record', elem_JQ = '#tblDataExists') {
        await $(elem_JQ).on("click", classClick  , function () {
            const row = $(this).closest("tr");
    ...
    //this for only return true or false
    isValidate = elem => {
        if (typeof elem.value.trim() === 'undefined' || elem.value.trim() === null || elem.value.trim() === '') {
            return false;
        }
        else {
            return true;
